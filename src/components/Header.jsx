...
const SearchInput = ({ placholder, icon, value, setValue, styles }) => {
      ...
      >
        <div className="w-full z-10">
          ...
          <div className="w-full flex items-center justify-around bg-white px-2 md:px-5 py-2.5 md:py-6 shadow-2xl rounded-full">
            <SearchInput
              placholder="Jobtitle or Keywords"
            ...
            />
            <SearchInput
              placholder="Add Country or City"
              icon={<CiLocationOn />}
              className="text-gray-600 text-xl"
              value={location}
                ...
                <div className="w-full lg:1/2 flex flex-wrap gap-3 md:gap-6 py-10 md:py-14">
                    {
                        popularSearch.map((search, index) => (
                            <span key={index}
                                className="bg-[#1d4fd826] text-[#1d4ed8] py-1 px-2 rounded-full text-sm md:text-base cursor-pointer"
                            >{search}</span>
                        ))
                    }
                </div>
